...
class CoachAthleteMessageList extends StatefulWidget {
  const CoachAthleteMessageList({super.key});

  @override
  State<CoachAthleteMessageList> createState() => _CoachAthleteMessageListState();
}

class _CoachAthleteMessageListState extends State<CoachAthleteMessageList> {
  ...
  Future getMessages() async {
    ...
    // coachList = await supabase.from('coach_profile').select();
    // for (var i = 0; i < coachList.length; i++) {
    //   String coachId = coachList[i]['coach_user_id'];
    //   if (messageList.contains(coachId)) {
    //     messageList.remove(coachId);
    //   }
    // }
    setState(() {
      isLoading = false;
